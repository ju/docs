...
<Info>Most commands can be run directly using Discord slash commands—look for the <Badge icon="slash-forward" color="green" size="sm" shape="pill"></Badge> badge! You don't need to use the `tickets` prefix (e.g., use `/claim` instead of `tickets claim`).</Info>
...
These commands let administrators configure the structure of the tickets system, as well as peek into ticket stats.
