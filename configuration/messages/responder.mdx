...
The `add` subcommand will grant roles to members and the `remove` subcommand will remove roles from members. 
...
```javascript Example
,autoresponder role (add or remove) Verified Verify
```
</CodeGroup>
<Frame>
