...
Permissions need to be rewrote in `snake_case` format, such as `ban_members` or `kick_members`.

<Frame>
  <img src="/images/configuration/moderation/permissions.png" />
</Frame>
...
<Tabs>
    <Tab title="Moderator">
        ...
        ```javascript
        ,fakepermissions grant (moderator role) manage_messages
        ,fakepermissions grant (moderator role) moderate_members
        ,fakepermissions grant (moderator role) manage_nicknames
        ,fakepermissions grant (moderator role) kick_members
        ```
        </Tab>
